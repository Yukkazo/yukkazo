...
# _____________________________________________________________________________________
            ...
            for idx, invoice in enumerate(invoices):
                if obj.type == 'purchase':
                    ...
                    if account_payment:
                        sheet.write(row, 5, account_payment.name, line)
                    ...
                    sheet.write(row, 25, invoice.amount_residual, line)
                    ###### IGTF
            ...
            if len(retenciones) > 0 and obj.type == 'purchase':
                ...
                for reten in sorted(retenciones, key=lambda x: x.date):
                    ...
                    # Documento afectado
                    if len(reten.reconciled_invoice_ids) >= 1:
                        total_base_imponible_contribuyente_16 += reten.reconciled_invoice_ids[0].amount_untaxed
                        taxs = reten.reconciled_invoice_ids.line_ids[0].tax_ids
                        total_iva_16 =+ reten.amount
                        if taxs and len(taxs) >= 1:
                            tax = taxs[0].amount
                        else:
                            tax = ''
                        
                        sheet.write(row, 3, reten.reconciled_invoice_ids[0].name, line)
                        sheet.write(row, 4, reten.reconciled_invoice_ids[0].l10n_ve_document_number or '', line)
                        sheet.write(row, 6, reten.reconciled_invoice_ids[0].name, line)
                        sheet.write(row, 9, reten.reconciled_invoice_ids[0].amount_total, line)
                        sheet.write(row, 15, reten.reconciled_invoice_ids[0].amount_untaxed, line)
                        sheet.write(row, 16, tax, line)
                        sheet.write(row, 17, reten.amount, line)
                    else:
                        sheet.write(row, 3, '', line)
                        sheet.write(row, 4, '', line)
                        sheet.write(row, 6, reten.reconciled_invoice_ids.name, line)
            ...
            # Totales de compras
                ...
                total_iva_16_retenido += total_amount_residual
                sheet.write((row + 5), 13, round(total_base_imponible_16,2), line)
                sheet.write((row + 5), 14, total_iva_16, line)
                sheet.write((row + 5), 15, total_amount_residual, line)
